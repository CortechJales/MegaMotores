...
class OrdemDeServicoController:
    ...
    def create_table(self):
        sql = '''CREATE TABLE IF NOT EXISTS ordens_servico (
                      ...
                      ativo BOOLEAN,
                      orcamento BOOLEAN,
                      orcamento_passado DATE,
                      orcamento_aprovado DATE,
    ...
    def ListarTodasOrdemServico(self, tipo, orcamento):
        query = '''SELECT 
                    ...
                    printf('%.2f', CAST(ordens_servico.mao_de_obra AS REAL)) AS mao_de_obra,
                    CASE 
                        WHEN COALESCE(SUM(CAST(itens_ordem.valor_unitario AS REAL) * itens_ordem.quantidade), 0) = 0 
                            THEN printf('%.2f', CAST(ordens_servico.mao_de_obra AS REAL))
                        ELSE printf('%.2f', ROUND(CAST(ordens_servico.mao_de_obra AS REAL) + SUM(CAST(itens_ordem.valor_unitario AS REAL) * itens_ordem.quantidade), 2))
                    END AS valor_total,
    ...
    def ListarOrdemServico(self, id):
        query = '''SELECT 
            ...
            ordens_servico.data_inicio,
            ordens_servico.data_final,
            printf('%.2f', CAST(REPLACE(ordens_servico.mao_de_obra, ',', '.') AS REAL)) AS mao_de_obra,
            CASE 
                WHEN COALESCE(SUM(CAST(REPLACE(itens_ordem.valor_unitario, ',', '.') AS REAL) * itens_ordem.quantidade), 0) = 0 
                    THEN printf('%.2f', CAST(REPLACE(ordens_servico.mao_de_obra, ',', '.') AS REAL))
                ELSE printf('%.2f', 
                    ROUND(
                        CAST(REPLACE(ordens_servico.mao_de_obra, ',', '.') AS REAL) + 
                        SUM(CAST(REPLACE(itens_ordem.valor_unitario, ',', '.') AS REAL) * itens_ordem.quantidade),
                    2)
                )
            END AS valor_total,
            ordens_servico.observacao,
        ...
        LEFT JOIN 
            itens_ordem ON ordens_servico.id = itens_ordem.ordem_id 
        WHERE 
            ordens_servico.ativo = 1 
            AND ordens_servico.id = ? 
        GROUP BY 
            ordens_servico.id;
                '''
        data = (id,)
    ...
    def FiltrarOrdemServico(self, tipo, orcamento, aberto):
        ...
        WHERE 
            ordens_servico.ativo = ? 
            AND ordens_servico.orcamento = ? 
            AND ordens_servico.fechada = ? 
        GROUP BY 
            ordens_servico.id;
                '''
    ...
    def CadastrarOrdemServico(self, cliente, equipamento, data_inicio, mao_de_obra, observacao, orcamento):
        query = '''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        '''
        mao_de_obra = float(str(mao_de_obra).replace(',', '.'))
    ...
    def ValidarOrdemPassadaAoCliente(self, id):
        ...
        return bool(result and result[0][0] is not None)
    ...
    def CalcularTotal(self, id):
        query = '''
            SELECT   
                ROUND(CAST(os.mao_de_obra AS REAL) + COALESCE(SUM(CAST(p.valor AS REAL) * io.quantidade), 0), 2) AS valor_total_ordem
            FROM 
    ...
    def CarregarImpressaoOrdem(self, id):
        query = '''SELECT 
            ...
            printf('%.2f',
                CAST(REPLACE(ordens_servico.mao_de_obra, ',', '.') AS REAL) +
                COALESCE(SUM(CAST(REPLACE(itens_ordem.valor_unitario, ',', '.') AS REAL) * itens_ordem.quantidade), 0)
            ) AS valor_total,
            printf('%.2f',
                COALESCE(SUM(CAST(REPLACE(itens_ordem.valor_unitario, ',', '.') AS REAL) * itens_ordem.quantidade), 0)
            ) AS total_itens_ordem,
            ordens_servico.observacao,
            ordens_servico.orcamento_passado,
            ordens_servico.orcamento_aprovado
        FROM 
